...
| **Sound RAM** | 8 MB @ `0x00800000-0x00ffffff` | 2 MB @ `0x00800000-0x009fffff` | **Quarter the sound RAM — but the game FITS (measured).** Write-truth measure (Phase 5, `docs/kb/phase2-measurements.md`): the game writes only ARAM `0x000000-0x1fffff` = **exactly 2 MB, zero bytes above**, loaded once at boot as a fixed bank. It already targets a ≤2 MB sound config → **no sample cuts needed on DC.** `naomi.cpp:1306`; `dccons.cpp:158` |
